...
create procedure get_last_notification_data_vars
(
	out last_notification_data_id int
)
begin
	select id
	into last_notification_data_id
	from tbl_notification_data
	order by id desc limit 1;

	if(last_notification_data_id is null) then
		set last_notification_data_id := 0;
	end if;
end$
...
begin
	...
	call get_last_notification_data_vars(@last_notification_data_id);
	insert into tbl_notification
		(id_subscription, id_content, seen, creation_timestamp)
		select id, last_content_id, false, now()
	...
	insert into tbl_notification_user
		...
		select id, id_subscription
		from tbl_notification_data
		where id > @last_notification_data_id;
end$
